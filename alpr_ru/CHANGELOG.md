...
- Исправлен зависающий прямой Dahua `VideoMotion` stream: подключение больше не считается живым бесконечно при молча зависшем TCP.
- Dahua heartbeat остаётся 5 секунд, но read timeout теперь 20 секунд; если поток перестал отдавать данные, App автоматически переподключается.
- В статус Dahua добавлены `last_heartbeat_at` и `last_stream_at` для диагностики живого event-stream.
- Парсер строк Dahua теперь делает `strip()` перед обработкой, чтобы не зависеть от CRLF/пробелов multipart-потока.
- `VideoMotion Start` по-прежнему запускает распознавание, `Stop` только фиксируется в статусе.
